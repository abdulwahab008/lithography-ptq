...
class PTQSLQuantConv2d(MinMaxQuantConv2d):
    ...
    def calibration_step2(self, x):
        ...
        weight_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1) * self.w_interval.unsqueeze(0) # shape: eq_n,n_V,1,n_H,1
        input_interval_candidates =  torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1) * self.a_interval # shape: nq_n,1,1,1,1
...
class BatchingEasyQuantConv2d(PTQSLQuantConv2d):
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0,self.calib_size,self.calib_batch_size):
            ...
            x_ = self.raw_input[b_st:b_ed].to(DEVICE)
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        ...
        for b_st in range(0,self.calib_size,self.calib_batch_size):
            ...
            raw_out = self.raw_out[b_st:b_ed].to(DEVICE).unsqueeze(1) # shape: b,1,oc,fw,fh
            raw_grad = self.raw_grad[b_st:b_ed].to(DEVICE)
            similarities = []
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                p_ed = min(self.eq_n, p_st+self.parallel_eq_n)
    ...
    def _search_best_a_interval(self, input_interval_candidates):
        ...
        for b_st in range(0,self.calib_size,self.calib_batch_size):
            b_ed = min(self.calib_size, b_st+self.calib_batch_size)
            x = self.raw_input[b_st:b_ed].to(DEVICE)
            raw_out = self.raw_out[b_st:b_ed].to(DEVICE).unsqueeze(0) # shape: 1,b,oc,fw,fh
            raw_grad = self.raw_grad[b_st:b_ed].to(DEVICE)
            similarities = []
            for p_st in range(0,self.eq_n,self.parallel_eq_n):
                p_ed = min(self.eq_n, p_st+self.parallel_eq_n)
    ...
    def calibration_step2(self):
        ...
        weight_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1) * self.w_interval # shape: eq_n,1,1,1,1
        input_interval_candidates =  torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1) * self.a_interval # shape: eq_n,1,1,1,1
        for e in range(self.search_round):
            # search for best weight interval
...
class ChannelwiseBatchingQuantConv2d(PTQSLQuantConv2d):
    ...
    def calibration_step2(self):
        self._initialize_calib_parameters()
        self._initialize_intervals()
        weight_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1) * self.w_interval.unsqueeze(0) # shape: eq_n,oc,1,1,1
        input_interval_candidates =  torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1) * self.a_interval # shape: eq_n,1,1,1,1
        for e in range(self.search_round):
            # search for best weight interval
            self._search_best_w_interval(weight_interval_candidates)

...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def calibration_step2(self, A, B):
        ...
        A_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1,1,1,1) * self.A_interval.unsqueeze(0)
        B_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1,1,1,1) * self.B_interval.unsqueeze(0)
...
class SoSPTQSLQuantMatMul(PTQSLQuantMatMul):
    ...
    def calibration_step2(self, A, B):
        ...
        # prepare weight intervals and similarities
        A_split_candidates = torch.tensor([2**(-i) for i in range(20)]).to(DEVICE)
        # split_eq_alpha, split_eq_beta, split_eq_n = 0.002, 0.03, 50
        # A_split_candidates = torch.tensor([split_eq_alpha + (split_eq_beta- split_eq_alpha)*i/split_eq_n for i in range(split_eq_n + 1)]).to(DEVICE)
        B_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(DEVICE).view(-1,1,1,1,1,1,1,1) * self.B_interval.unsqueeze(0)
...
class PTQSLBatchingQuantMatMul(PTQSLQuantMatMul):
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0,self.calib_size,self.calib_batch_size):
            ...
            A, B = self.raw_input[0][b_st:b_ed].to(DEVICE), self.raw_input[1][b_st:b_ed].to(DEVICE)
            if self.init_layerwise:
    ...
    def _search_best_A_interval(self, A_interval_candidates):
        ...
        for v, h in product(range(self.n_V_A), range(self.n_H_A)):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                b_ed = min(self.calib_size, b_st + self.calib_batch_size)
                A = self.raw_input[0][b_st:b_ed].to(DEVICE)
                A_pad = F.pad(A, [0,self.pad_cols_A,0,self.pad_rows_A,0,self.pad_groups_A]).unsqueeze(0).view(1,-1,self.n_G_A,self.crb_groups_A,self.n_V_A,self.crb_rows_A,self.n_H_A,self.crb_cols_A)
                B = self.raw_input[1][b_st:b_ed].to(DEVICE)
                B_sim = self.quant_input_B(B).unsqueeze(0) # shape: 1,b,H,dim2,dim3
                raw_out = self.raw_out[b_st:b_ed].unsqueeze(0).to(DEVICE)
...
class SoSPTQSLBatchingQuantMatMul(PTQSLBatchingQuantMatMul):
    ...
    def _search_best_A_interval(self, split_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            b_ed = min(self.calib_size, b_st + self.calib_batch_size)
            A = self.raw_input[0][b_st:b_ed].unsqueeze(0).to(DEVICE)
            B = self.raw_input[1][b_st:b_ed].unsqueeze(0).to(DEVICE)
            B_sim = B
            raw_out = self.raw_out[b_st:b_ed].unsqueeze(0).to(DEVICE)
            raw_grad = self.raw_grad[b_st:b_ed].to(DEVICE)
            similarities = []

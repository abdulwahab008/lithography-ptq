            ...
            if hasattr(module, "calibrated"):
                ...
                with torch.no_grad():
                    if isinstance(module, MinMaxQuantLinear):
                        module.forward(module.raw_input.to(DEVICE))
                    elif isinstance(module, MinMaxQuantConv2d):
                        module.forward(module.raw_input.to(DEVICE))
                    elif isinstance(module, MinMaxQuantMatMul):
                        module.forward(module.raw_input[0].to(DEVICE), module.raw_input[1].to(DEVICE))
        ...
        for name, module in q:
            ...
            for inp, target in self.calib_loader:
                for batch_st in range(0,self.calib_loader.batch_size,self.batch_size):
                    self.net.zero_grad()
                    inp_ = inp[batch_st:batch_st+self.batch_size].to(DEVICE)
                    self.net(inp_)
                del inp, target
                torch.cuda.empty_cache()
    ...
    def quant_calib(self):
        ...
        with torch.no_grad():
            for inp, _ in self.calib_loader:
                raw_pred = self.net(inp.to(DEVICE))
        ...
        for name, module in q:
            ...
            with torch.no_grad():
                if isinstance(module, MinMaxQuantLinear):
                    module.calibration_step2(module.raw_input.to(DEVICE))
                if isinstance(module, MinMaxQuantConv2d):
                    module.calibration_step2(module.raw_input.to(DEVICE))
                if isinstance(module, MinMaxQuantMatMul):
                    module.calibration_step2(module.raw_input[0].to(DEVICE), module.raw_input[1].to(DEVICE))
                torch.cuda.empty_cache()
